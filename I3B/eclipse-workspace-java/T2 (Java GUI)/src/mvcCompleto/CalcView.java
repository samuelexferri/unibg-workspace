...
class CalcView extends JFrame implements Observer {
    ...
    CalcView(CalcModel model) {
    	...
    	m_model = model;
    	//il model implementa Observable, aggiungo al modello un osservatore (la view stessa)
    ...
    /*
     * I metodi seguenti servono a chi detiene un riferimento alla view (il controller)
     * Se non ci fossero, il controller dovrebbe avere un riferimento esplicito a tutti gli elementi
     * della view per poter svolgere operazioni. In questo modo, invece, è sufficiente avere il riferimento
     * all'intera classe CalcView.
     */
    //Getter per rendere disponibile all'esterno il valore del campo testo del textField
    String getUserInput() {
	...
	//Ereditato da Observer, chiama il metodo update definito localmente quando l'osservato (il modello)
    ...
    //in questo caso è l'azione compiuta dalla GUI quando il model (che è osservato)
    //effettua una notifica
	private void update() {
