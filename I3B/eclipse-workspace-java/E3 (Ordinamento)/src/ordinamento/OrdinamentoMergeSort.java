...
public class OrdinamentoMergeSort implements Ordinamento {
	...
	/**
	 * Ordina l'array di input utilizzando l'algoritmo di merge sort. L'ordinamento
	 * avviene richiamando internamente il metodo ricorsivo <code>mergeSort</code>.
	 * 
	 ...
	 * <code>mergeSort</code>, per l'ordinamento di sottosequenze di <code>A</code>,
	 * ed il metodo <code>merge</code>, per la fusione di sottosequenze ordinate.
	 * 
	 * @param A l'array da ordinare
	...
	/**
	 * Fonde due sottosequenze di elementi consecutivi appartenenti ad uno stesso
	 * array di input in un'unica sequenza ordinata. La sequenza prodotta dalla
	 * fusione viene collocata nelle posizioni originariamente occupate dalle due
	 * sottosequenze. Assume che gli elementi delle due sottosequenze siano ordinati
	 * 
	...
	private void merge(Comparable A[], int i1, int f1, int f2) {
		...
		if (i1 <= f1) // La sottosequenza in cui rimangono elementi � A[i1..f1]
